...
| ✓ | No | モジュール名 | 種別 | OLE ストリーム | 用途 |
| --- | --- | --- | --- | --- | --- |
| ✓ | 1 | **ThisWorkbook** | .cls | VBA/ThisWorkbook | ブック Open / Close イベント処理 |
|  | 2 | **Sheet1** | .cls | VBA/Sheet1 | 「出荷日情報」シートクラス（コード空） |
|  | 3 | **Sheet2** | .cls | VBA/Sheet2 | 「照射データ」シートクラス（コード空） |
|  | 4 | **Sheet4** | .cls | VBA/Sheet4 | 「完了予定時間」シートクラス（コード空） |
|  | 5 | **Sheet5** | .cls | VBA/Sheet5 | 「線源情報」シートクラス（コード空） |
| ✓ | 6 | **Ex抽出処理** | .bas | VBA/Ex抽出処理 | メインロジック — DB から照射データ・線源情報を取得し完了予想時刻を算出 |
| ✓ | 7 | **SQL_Execution** | .bas | VBA/SQL_Execution | ADO+ODBC によるDB接続・SQL実行ユーティリティ |
|  | 8 | **Ex画面クリア** | .bas | VBA/Ex画面クリア | 画面初期化処理 |
|  | 9 | **Ex終了照射情報** | .bas | VBA/Ex終了照射情報 | ブック終了処理 |
|  | 10 | **Ex出荷日情報読込Ric1** | .bas | VBA/Ex出荷日情報読込Ric1 | 出荷日情報の DB 読込 |
|  | 11 | **FunctionR1** | .bas | VBA/FunctionR1 | 日付変換ユーティリティ関数 |
|  | 12 | **GetPathRic1Jyou** | .bas | VBA/GetPathRic1Jyou | 外部パス設定ファイル読込 |
...
| ✓ | No | ボタン名 | マクロ | 用途 |
| --- | --- | --- | --- | --- |
| ✓ | 1 | 「最新情報」 | `[0]!Kousinn` | DB から最新の照射データ・線源情報・出荷日情報を取得し画面を更新 |
| ✓ | 2 | 「終了」 | `[0]!照射情報終了処理` | ブックを閉じる（保存なし） |
...
| ✓ | No | テーブル名 | 区分 | アクセス種別 | 用途 |
| --- | --- | --- | --- | --- | --- |
| ✓ | 1 | `SYOUK1` | 参照 (SELECT) | R | 照射管理データ（未照射品一覧） |
| ✓ | 2 | `RIC.SENGNR1` | 参照 (SELECT) | R | 線源タイマー情報（直近データ取得） |
| ✓ | 3 | `ExKeikakuX` | 参照 (SELECT) | R | 出荷計画（出荷日・備考・出荷方法） |

本ブックは参照専用であり、DB への INSERT / UPDATE / DELETE は行わない。

### 8.3 SQL 一覧

| No | モジュール | SQL 概要 | テーブル |
| --- | --- | --- | --- |
...
| No | コードネーム | workbook.xml シート名 | worksheet XML |
| --- | --- | --- | --- |
| 1 | Sheet4 | 「完了予定時間」 | xl/worksheets/sheet1.xml |
| 2 | Sheet2 | 「照射データ」 | xl/worksheets/sheet2.xml |
| 3 | Sheet5 | 「線源情報」 | xl/worksheets/sheet3.xml |
| 4 | Sheet1 | 「出荷日情報」 | xl/worksheets/sheet4.xml |
