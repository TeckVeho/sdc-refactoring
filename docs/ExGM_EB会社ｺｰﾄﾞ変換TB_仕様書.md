...
| ✓ | **Sheet1.cls** | .cls | 1 | 「登録」シートイベント |
|   | **共通変数.bas** | .bas | — | Public 変数定義 |
| ✓ | **SQL_Execution.bas** | .bas | — | ADO/ODBC DB接続・SQL実行基盤 |
| ✓ | **登録.bas** | .bas | — | 登録更新処理 |
| ✓ | **リンク解消.bas** | .bas | — | ペア削除処理 |
|   | **クリア.bas** | .bas | — | 画面クリア処理 |
| ✓ | **該当表示.bas** | .bas | — | 会社名表示・既存紐付チェック |
| ✓ | **全データ表示.bas** | .bas | — | 一覧表示処理 |
...
| マクロ名 | ショートカット | 処理概要 |
|---|---|---|
| `画面クリア()` | **Ctrl+E** | 入力欄・一覧領域をクリアし初期状態に戻す |
...
| ✓ | # | モジュール | プロシージャ | 種別 | 概要 |
|---|---|---|---|---|---|
| ✓ | 1 | **ThisWorkbook.cls** | `Workbook_Open()` | Event | シート保護・画面クリア・一覧表示・フィルター有効化 |
| ✓ | 2 | **ThisWorkbook.cls** | `Workbook_BeforeClose()` | Event | 保存ダイアログ抑止 |
| ✓ | 3 | **Sheet1.cls** | `Worksheet_Change()` | Event | セル変更時に会社名DB検索・既存紐付チェック |
|   | 4 | **共通変数.bas** | （宣言のみ） | — | Public変数定義（mpGMKaicd, mpEBKaicd, mpKaiName） |
| ✓ | 5 | **SQL_Execution.bas** | `Open_oraconDB()` | Sub | Oracle DB接続（ODBC） |
| ✓ | 6 | **SQL_Execution.bas** | `SQL_Exe()` | Sub | SQL実行 |
| ✓ | 7 | **SQL_Execution.bas** | `SQL_INSERT_UPDATE()` | Sub | INSERT/UPDATE汎用処理 |
| ✓ | 8 | **SQL_Execution.bas** | `SQL_Delete()` | Sub | DELETE汎用処理 |
| ✓ | 9 | **SQL_Execution.bas** | `Disp_Sheet()` | Sub | SQL結果をシートに出力 |
| ✓ | 10 | **SQL_Execution.bas** | `Set_Array()` | Sub | SQL結果を配列に格納 |
| ✓ | 11 | **登録.bas** | `登録更新()` | Sub | ガンマ/EBコード組み合わせをDBに登録・更新 |
| ✓ | 12 | **リンク解消.bas** | `PairDelete()` | Sub | ガンマ/EBコード紐付けをDBから削除 |
| ✓ | 13 | **クリア.bas** | `画面クリア()` | Sub | 入力欄・一覧領域クリア（ショートカット Ctrl+E） |
|   | 14 | **クリア.bas** | `EventOn()` | Sub | イベント強制再有効化（デバッグ用） |
| ✓ | 15 | **該当表示.bas** | `会社名表示()` | Sub | ガンマコードで会社名・住所をDB検索表示 |
| ✓ | 16 | **該当表示.bas** | `既存紐付チェック()` | Sub | EBコードの既存リンク有無を確認 |
| ✓ | 17 | **全データ表示.bas** | `AllHyouji()` | Sub | DB登録済みデータを一覧表示 |
...
| `tokumst` | 顧客マスタ（※推論: テーブル名 TOKU+MST から推定。VBAには「顧客コード」の文言あり、「顧客マスタ」の記載なし） | `kaisyacd`, `coname`（会社名）, `jyus1`（住所1）, `jyus2`（住所2） |
