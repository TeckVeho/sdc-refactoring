...
| --- | --- | --- | --- | --- | --- | --- |
|  | 1 | A1 | — | ラベル | システム識別子（`Ex4`） | システム識別子（`Ex4`） |
|  | 2 | B2 | — | ラベル | タイトル「顧客コード変換テーブル登録」 | タイトル「顧客コード変換テーブル登録」 |
|  | 3 | B5 | — | ラベル | 入力ヘッダー「ガンマ顧客コード」 | 入力ヘッダー「ガンマ顧客コード」 |
|  | 4 | C5 | — | ラベル | 入力ヘッダー「EB顧客コード」 | 入力ヘッダー「EB顧客コード」 |
|  | 5 | D5 | — | ラベル | 入力ヘッダー「会社名」 | 入力ヘッダー「会社名」 |
|  | 6 | E5 | — | ラベル | 入力ヘッダー「住所１」 | 入力ヘッダー「住所１」 |
|  | 7 | F5 | — | ラベル | 入力ヘッダー「住所2」 | 入力ヘッダー「住所2」 |
| ✓ | 8 | B6 | `GMTNo` / `Touroku`(B6:F6) | 入力 | ガンマ顧客コード入力欄 | ガンマ顧客コード入力欄 |
| ✓ | 9 | C6 | `EBTNo` / `Touroku`(B6:F6) | 入力 | EB顧客コード入力欄 | EB顧客コード入力欄 |
| ✓ | 10 | D6 | `KaiName` / `Touroku`(B6:F6) | VBA代入 | 会社名表示欄（DB参照） | 会社名表示欄（DB参照） |
| ✓ | 11 | E6 | `Jyuusyo1` / `Touroku`(B6:F6) | VBA代入 | 住所１表示欄（DB参照） | 住所１表示欄（DB参照） |
| ✓ | 12 | F6 | `Jyuusyo2` / `Touroku`(B6:F6) | VBA代入 | 住所２表示欄（DB参照） | 住所２表示欄（DB参照） |
|  | 13 | B7 | — | ラベル | セクションラベル「登録済一覧」 | セクションラベル「登録済一覧」 |
|  | 14 | B9 | — | ラベル | 一覧ヘッダー「ガンマ顧客コード」 | 一覧ヘッダー「ガンマ顧客コード」 |
|  | 15 | C9 | — | ラベル | 一覧ヘッダー「EB顧客コード」 | 一覧ヘッダー「EB顧客コード」 |
|  | 16 | D9 | — | ラベル | 一覧ヘッダー「会社名」 | 一覧ヘッダー「会社名」 |
|  | 17 | E9 | — | ラベル | 一覧ヘッダー「住所１」 | 一覧ヘッダー「住所１」 |
|  | 18 | F9 | — | ラベル | 一覧ヘッダー「住所2」 | 一覧ヘッダー「住所2」 |
| ✓ | 19 | B10:F193 | `AllData` | VBA代入 | DB取得データ表示領域（最大184件） | DB取得データ表示領域（最大184件） |
...
| --- | --- | --- | --- | --- | --- |
|  | 1 | 登録 | 登録 | `登録更新()` | 入力したガンマ/EBコードをDBの`kcdcnvmst`テーブルに登録または更新する |
|  | 2 | 登録 | リンク解消 | `PairDelete()` | 入力したガンマ/EBコードの紐付けをDBから削除する |
|  | 3 | 登録 | 終了 | `Bookを閉じる()` | ブックを保存せずに閉じる（他のブックがなければExcel終了） |
...
| DSN 名 | UID | PWD | 用途 |
| --- | --- | --- | --- |
| `ricdb` | `ric` | `t6101` | 照射管理システムDB — 顧客コード変換・得意先マスタの参照 |
...
| --- | --- | --- | --- | --- | --- | --- |
| ✓ | 1 | `kcdcnvmst` | **参照＋更新** | ガンマ/EB顧客コード変換テーブル | `kaisyacd`, `ebkaisyacd` | 更新: `kaisyacd`, `ebkaisyacd`（`登録更新()` から INSERT/UPDATE、`PairDelete()` から DELETE） |
|  | 2 | `tokumst` | 参照 | 顧客マスタ（※推論: テーブル名 TOKU+MST から推定。VBAには「顧客コード」の文言あり、「顧客マスタ」の記載なし） | `kaisyacd` | `kaisyacd`, `coname`, `jyus1`, `jyus2` |

> **「キー列」の定義**: JOIN 条件または UPDATE/DELETE の WHERE 句で使用される列を示す。

### 8.3 SQL 一覧

SQL 本文は 6 章に記載済みのため、ここでは再掲せず参照のみとする。

#### 8.3.1 登録更新（`登録更新()` / **登録.bas**）

6.5 参照。

#### 8.3.2 リンク解消（`PairDelete()` / **リンク解消.bas**）

6.6 参照。

#### 8.3.3 会社名表示（`会社名表示()` / **該当表示.bas**）

6.8 参照。

#### 8.3.4 既存紐付チェック（`既存紐付チェック()` / **該当表示.bas**）

6.8 参照。

#### 8.3.5 全データ表示（`AllHyouji()` / **全データ表示.bas**）
...
| 2 | 上記 | 📊 シート保護設定（UIのみ許可） | フィルタ / ソートを許可 |
| 3 | 上記 | 📊 `画面クリア()` / **クリア.bas** | `Touroku` / `AllData` をクリア |
| 4 | 上記 | 🗄️ `AllHyouji()` / **全データ表示.bas** | `kcdcnvmst` + `tokumst` を SELECT |
| 5 | 上記 | 📊 B10以降に一覧表示 → オートフィルター → `B6` にフォーカス | 入力待ち |
...
└─ 📄 Workbook_Open              [ThisWorkbook.cls]
   ├─ 📊 シート保護（UIのみ、フィルタ/ソート許可）
   ├─ 📊 画面クリア               [クリア.bas]（Touroku / AllData）
   └─ 🗄️ AllHyouji               [全データ表示.bas]
      └─ 🗄️ kcdcnvmst + tokumst SELECT → 📊 B10以降に一覧 → B6.Select
```
...
| No | 起点 | 処理 | 結果 |
| --- | --- | --- | --- |
| 1 | 📊 `B6` にガンマ顧客コード入力 | 📄 `Worksheet_Change()` / **Sheet1.cls** | 4桁ゼロ埋め |
| 2 | 上記 | 🗄️ `会社名表示()` / **該当表示.bas** | `tokumst` + `kcdcnvmst` を SELECT |
| 3 | 上記 | 📊 `D6` / `E6` / `F6` / `C6` に会社名・住所・既存EBを表示 | 入力行を更新 |
| 4 | 📊 `C6` にEB顧客コード入力 | 📄 `Worksheet_Change()` / **Sheet1.cls** | 3桁ゼロ埋め |
| 5 | 上記 | 🗄️ `既存紐付チェック()` / **該当表示.bas** | 重複を確認 |
| 6 | 📊（登録 Click） | 📄 `登録更新()` / **登録.bas** | チェック SQL ①②③ |
| 7 | 上記 | 🗄️ `SQL_INSERT_UPDATE()` / **SQL_Execution.bas** | `kcdcnvmst` を INSERT/UPDATE |
| 8 | 上記 | 🗄️ `AllHyouji()` → 📊 一覧再表示 | B10以降を更新 |
...
└─ 📄 Worksheet_Change           [Sheet1.cls]
   ├─ 🗄️ 会社名表示              [該当表示.bas]
   │   └─ 🗄️ tokumst + kcdcnvmst SELECT → 📊 D6/E6/F6/C6
   └─ （C6 にEB顧客コード入力）
      └─ 📄 Worksheet_Change     [Sheet1.cls]
         ├─ 🗄️ 既存紐付チェック  [該当表示.bas]
         └─ 📊（登録 Click）
            └─ 📄 登録更新        [登録.bas]
               ├─ 🗄️ チェックSQL ①②③
               ├─ 🗄️ SQL_INSERT_UPDATE [SQL_Execution.bas]（kcdcnvmst）
               └─ 🗄️ AllHyouji → 📊 一覧再表示
```
...
| No | 起点 | 処理 | 結果 |
| --- | --- | --- | --- |
| 1 | 📊（リンク解消 Click） | 📄 `PairDelete()` / **リンク解消.bas** | 存在確認を開始 |
| 2 | 上記 | 🗄️ `kcdcnvmst` を SELECT | ペアの有無を確認 |
| 3 | 上記 | 🗄️ `SQL_Delete()` / **SQL_Execution.bas** | `kcdcnvmst` から DELETE |
| 4 | 上記 | 📊 `Touroku` クリア → 🗄️ `AllHyouji()` → 📊 一覧再表示 | B10以降を更新 |

#### ツリー図（補助）

```
（リンク解消 Click）
└─ 📄 PairDelete                 [リンク解消.bas]
   ├─ 🗄️ SELECT kcdcnvmst（ペア存在確認）
   ├─ 🗄️ SQL_Delete              [SQL_Execution.bas]
   ├─ 📊 Touroku クリア
   └─ 🗄️ AllHyouji → 📊 一覧再表示
```
