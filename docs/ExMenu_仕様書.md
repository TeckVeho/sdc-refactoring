...
| --- | --- | --- | --- | --- | --- | --- |
|  | 1 | Syainn | Label | 社員 | — | 社員番号ラベル |
|  | 2 | Label1 | Label | — | — | 補助ラベル |
| ✓ | 3 | SyainnNo | TextBox | — | — | 社員番号入力 |
| ✓ | 4 | PassWord | TextBox | — | — | パスワード入力 |
| ✓ | 5 | Kakuninn | CommandButton | 確認 | — | 資格照合実行 |
| ✓ | 6 | CommandButton1 | CommandButton | キャンセル | — | フォームを閉じる |
...
| 3 | Suspicious | サーバーからローカルへ最大 30 アプリを自動コピー | 低：olevba 検出項目 |
| 4 | IOC | 旧サーバー IP（コメントアウト済み UNC パス内）。現在は `C:\ラジエ工業\ExRicSys\`〈EXメニュー配置フォルダ〉 | 低：社内アドレスの露出 |
| 5 | ファイル操作 | Open / Write / Kill / MkDir 等を使用 | 中：ローカルファイルの作成・削除が可能 |

| No | 注意 | 内容 |
| --- | --- | --- |
| 1 | DB 認証情報 | 接続文字列に UID/PWD をハードコード: `DSN=ricdb;UID=ric;PWD=t6101` |
| 2 | ファイル自動コピー | 起動時にサーバーから最大 30 ファイルをローカルへ自動コピー |
| 3 | 社員パスワード | `SHAINMST.shask` を SQL 直結照合。ハッシュ化等の対策は不明 |
## スコープ外（本仕様書に含まないもの）
