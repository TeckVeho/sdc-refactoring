...
|---|---|---|---|---|---|
| ✓ | 「メニュー」 | 24 | 13 (M) | visible | —（シート自体の Visible は変更しない。ActiveWindow の見出し/グリッド等を非表示化） |
| ✓ | 「AprList」 | 38 | 15 (O) | visible | `AddSoft()` で「メニュー」から画面遷移（Select）/ `BackMenu()` で「メニュー」へ戻る |
...
| ✓ | フォーム名 | コントロール数 | 主用途 |
|---|---|---|---|
| ✓ | **SikakuKakuninn.frm** | 6 | メニュー登録時の資格確認（社員番号/パスワード入力） |
...
|---|---|---|---|---|
| ✓ | **ThisWorkbook.cls** | .cls | 3 | 起動/終了/保存イベント |
|   | **Sheet3.cls** | .cls | 0 | 「メニュー」シート（コードなし） |
|   | **Sheet5.cls** | .cls | 0 | 「AprList」シート（コードなし） |
| ✓ | **共通変数と開始処理.bas** | .bas | 2 | 起動時初期化・パス取得 |
| ✓ | **メニュー選択実行.bas** | .bas | 31 | Menu1〜30 起動・二重起動チェック |
| ✓ | **SQL_Execution.bas** | .bas | 6 | ADO/ODBC 接続・SQL 実行基盤 |
| ✓ | **ファイル登録_取込.bas** | .bas | 5 | DB 取込・メニュー登録・画面遷移 |
| ✓ | **お知らせメッセージ.bas** | .bas | 2 | 起動時お知らせ表示 |
|   | **Debug用.bas** | .bas | 2 | デバッグ用メニュークリア |
| ✓ | **SikakuKakuninn.frm** | .frm | 5 | 資格確認 UserForm |
...
|---|---|---|---|---|
| 「メニュー」 | シート自体は常時 visible。ActiveWindow の見出し・スクロールバー・タブ・グリッドを非表示 | `初期設定()` 実行後 | `AllCls()` 実行時（Debug 用） | **共通変数と開始処理.bas** / **Debug用.bas** |
| 「AprList」 | シート自体は常時 visible だが通常操作時は「メニュー」が前面 | 「メニュー」シートの【ファイルの追加・消去】クリック時（`AddSoft()`） | `BackMenu()` で「メニュー」シートへ戻る | **ファイル登録_取込.bas**（`AddSoft()` / `BackMenu()`） |
...
| ✓ | セル / オブジェクト | 名前付き範囲 | 種別 | 実態（値/数式/VBA代入） | 業務的意味 |
|---|---|---|---|---|---|
| ✓ | `A1` | `Debug` | 手動設定 | 空欄 or `*` | デバッグモードフラグ（空=通常、`*`=非表示モード。`AllCls` で参照） |
|   | `B2` | — | 設定値 | `V3M` | メニューバージョン表示 |
|   | `K1` | — | 数式 | `=NOW()` | 現在日時表示 |
| ✓ | Menu1〜Menu30 | — | VBA代入 | `初期設定()` で `mpAprD(i,2)` を Characters.Text に設定 | 各スロットのアプリ表示名。`-----` は利用不可 |
| ✓ | Menu1〜Menu30 | — | VBA代入 | `初期設定()` で OnAction=`選択N` を設定 | クリック時に `選択N()` を実行 |
...
|---|---|---|---|---|---|
|   | `B2` | — | 設定値 | `ﾌｧｲﾙ一覧` | シートタイトル |
| ✓ | `F2` | `AprSuu` | 数式 | `=COUNTA(D4:D33)` | 登録ファイル数（ファイル名が入っている行数） |
|   | `B3:O3` | — | 設定値 | 列ヘッダー | 各列の意味を示す見出し |
|   | `B4:B33` | — | 設定値 | 1〜30 | メニュー番号 |
| ✓ | `C4:C33` | `PGList` 先頭列 | 手動設定 / DB読込 | Path 名（50B 以内） | アプリ格納フォルダパス |
| ✓ | `D4:D33` | — | 手動設定 / DB読込 | ファイル名（50B 以内） | 起動対象 Excel ファイル名 |
| ✓ | `E4:E33` | — | 手動設定 / DB読込 | メニュー表示文字（100B 以内） | メニューボタンに表示する名称 |
| ✓ | `F4:F33` | — | 手動設定 / DB読込 | ソフト種別（20B 以内） | アプリ分類 |
| ✓ | `G4:G33` | — | 手動設定 / VBA代入 | 登録/非表示日 | `メニュー登録()` で `Now` を書込み |
| ✓ | `H4:H33` | — | 手動設定 / VBA代入 | 登録者（20B 以内） | 資格確認後の社員名 |
| ✓ | `I4:I33` | — | DB読込 | Path 名 | `メニュー取込()` が ExSeihinj から書込み |
| ✓ | `J4:J33` | — | DB読込 | ファイル名 | 同上 |
| ✓ | `K4:K33` | — | DB読込 | メニュー表示文字 | 同上 |
| ✓ | `L4:L33` | — | DB読込 | ソフト種別 | 同上 |
| ✓ | `M4:M33` | — | DB読込 | 登録日 | 同上 |
| ✓ | `N4:N33` | — | DB読込 | 登録者 | 同上 |
| ✓ | `O4:O33` | — | 数式 | `=IF(C& D& E& F <> I& J& K& L, FALSE, TRUE)` | 変更有無。False=変更あり（C〜F と I〜L の比較） |
...
| ✓ | 名前 | 参照先 | 業務的意味 |
|---|---|---|---|
| ✓ | `AprSuu` | AprList!$F$2 | 登録可能アプリ数の表示セル（`=COUNTA(D4:D33)`） |
| ✓ | `Debug` | メニュー!$A$1 | デバッグモードフラグ。`AllCls` / `Workbook_BeforeSave`(TEST) で参照 |
| ✓ | `PGList` | AprList!$C$4:$N$33 | アプリ一覧データ範囲。`初期設定` / `メニュー取込` / `メニュー登録Main` / `二重起動チェックと起動` が Row/Column オフセットで参照 |
...
#### 「AprList」シート（Form Control）

| ✓ | シート | ボタンラベル | 割り当てマクロ | 動作概要 |
|---|---|---|---|---|
| ✓ | 「AprList」 | ﾌｧｲﾙ登録 | `メニュー登録Main()` | 変更あり行を ExSEIHINJ に登録/消去 |
| ✓ | 「AprList」 | メニュー画面 | `BackMenu()` | 「メニュー」シートへ戻る |

#### 「メニュー」シート（Shape / FormControl）

##### 【ファイルの追加・消去】ボタン

| ✓ | シート | ボタンラベル | 割り当てマクロ | 動作概要 |
|---|---|---|---|---|
| ✓ | 「メニュー」 | ファイルの追加・消去 | `AddSoft()` | 「AprList」シートへ画面遷移し、ファイル登録・管理画面を表示 |
...
|---|---|---|---|---|
| ✓ | 「メニュー」 | Menu1 | `選択1()` | AprList No.1 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu2 | `選択2()` | AprList No.2 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu3 | `選択3()` | AprList No.3 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu4 | `選択4()` | AprList No.4 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu5 | `選択5()` | AprList No.5 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu6 | `選択6()` | AprList No.6 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu7 | `選択7()` | AprList No.7 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu8 | `選択8()` | AprList No.8 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu9 | `選択9()` | AprList No.9 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu10 | `選択10()` | AprList No.10 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu11 | `選択11()` | AprList No.11 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu12 | `選択12()` | AprList No.12 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu13 | `選択13()` | AprList No.13 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu14 | `選択14()` | AprList No.14 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu15 | `選択15()` | AprList No.15 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu16 | `選択16()` | AprList No.16 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu17 | `選択17()` | AprList No.17 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu18 | `選択18()` | AprList No.18 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu19 | `選択19()` | AprList No.19 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu20 | `選択20()` | AprList No.20 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu21 | `選択21()` | AprList No.21 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu22 | `選択22()` | AprList No.22 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu23 | `選択23()` | AprList No.23 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu24 | `選択24()` | AprList No.24 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu25 | `選択25()` | AprList No.25 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu26 | `選択26()` | AprList No.26 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu27 | `選択27()` | AprList No.27 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu28 | `選択28()` | AprList No.28 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu29 | `選択29()` | AprList No.29 のアプリを起動（`二重起動チェックと起動`） |
| ✓ | 「メニュー」 | Menu30 | `選択30()` | AprList No.30 のアプリを起動（`二重起動チェックと起動`） |

### 5.2 ユーザーフォーム上のボタン

| ✓ | フォーム | コントロール | キャプション | イベント | 呼び出すプロシージャ | 動作概要 |
|---|---|---|---|---|---|---|
| ✓ | **SikakuKakuninn.frm** | Kakuninn | 確認 | Click | `Kakuninn_Click()` → `SyainName()` | SHAINMST で資格照合。成功時 `mpSyaName` 設定してフォームを閉じる |
| ✓ | **SikakuKakuninn.frm** | CommandButton1 | キャンセル | Click | `CommandButton1_Click()` | フォームを閉じる（登録中止） |
...
| ✓ | モジュール | プロシージャ | 種別 | 概要 |
|---|---|---|---|---|
| ✓ | **ThisWorkbook.cls** | `Workbook_Open()` | Event | ブック起動時に `初期設定()` を呼び出す |
| ✓ | **ThisWorkbook.cls** | `Workbook_BeforeClose()` | Event | 終了時に Standard/Formatting ツールバーと数式バーを表示に戻す |
|  | **ThisWorkbook.cls** | `Workbook_BeforeSave()` | Event | 保存抑止ロジックはコメントアウト済み（TEST） |
|  | **Sheet3.cls** | — | — | 「メニュー」シート（コードなし） |
|  | **Sheet5.cls** | — | — | 「AprList」シート（コードなし） |
| ✓ | **メニュー選択実行.bas** | `選択1()` | Sub | Menu1 クリック時。`二重起動チェックと起動(1)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択2()` | Sub | Menu2 クリック時。`二重起動チェックと起動(2)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択3()` | Sub | Menu3 クリック時。`二重起動チェックと起動(3)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択4()` | Sub | Menu4 クリック時。`二重起動チェックと起動(4)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択5()` | Sub | Menu5 クリック時。`二重起動チェックと起動(5)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択6()` | Sub | Menu6 クリック時。`二重起動チェックと起動(6)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択7()` | Sub | Menu7 クリック時。`二重起動チェックと起動(7)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択8()` | Sub | Menu8 クリック時。`二重起動チェックと起動(8)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択9()` | Sub | Menu9 クリック時。`二重起動チェックと起動(9)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択10()` | Sub | Menu10 クリック時。`二重起動チェックと起動(10)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択11()` | Sub | Menu11 クリック時。`二重起動チェックと起動(11)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択12()` | Sub | Menu12 クリック時。`二重起動チェックと起動(12)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択13()` | Sub | Menu13 クリック時。`二重起動チェックと起動(13)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択14()` | Sub | Menu14 クリック時。`二重起動チェックと起動(14)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択15()` | Sub | Menu15 クリック時。`二重起動チェックと起動(15)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択16()` | Sub | Menu16 クリック時。`二重起動チェックと起動(16)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択17()` | Sub | Menu17 クリック時。`二重起動チェックと起動(17)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択18()` | Sub | Menu18 クリック時。`二重起動チェックと起動(18)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択19()` | Sub | Menu19 クリック時。`二重起動チェックと起動(19)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択20()` | Sub | Menu20 クリック時。`二重起動チェックと起動(20)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択21()` | Sub | Menu21 クリック時。`二重起動チェックと起動(21)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択22()` | Sub | Menu22 クリック時。`二重起動チェックと起動(22)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択23()` | Sub | Menu23 クリック時。`二重起動チェックと起動(23)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択24()` | Sub | Menu24 クリック時。`二重起動チェックと起動(24)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択25()` | Sub | Menu25 クリック時。`二重起動チェックと起動(25)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択26()` | Sub | Menu26 クリック時。`二重起動チェックと起動(26)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択27()` | Sub | Menu27 クリック時。`二重起動チェックと起動(27)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択28()` | Sub | Menu28 クリック時。`二重起動チェックと起動(28)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択29()` | Sub | Menu29 クリック時。`二重起動チェックと起動(29)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `選択30()` | Sub | Menu30 クリック時。`二重起動チェックと起動(30)` を呼び出す |
| ✓ | **メニュー選択実行.bas** | `二重起動チェックと起動()` | Sub | 二重起動チェック後、`Workbooks.Open` でアプリを起動 |
|  | **Debug用.bas** | `AllCls()` | Sub | デバッグ用。メニュー表示を解除し `PGList` をクリア |
|  | **Debug用.bas** | `DellMenu()` | Sub | Menu1〜Menu30 のラベルと OnAction を削除 |
| ✓ | **共通変数と開始処理.bas** | `初期設定()` | Sub | 起動時の全初期化（バージョン確認・取込・同期・メニュー構築・お知らせ） |
| ✓ | **共通変数と開始処理.bas** | `MenuGetPath()` | Function | `ExAprReadPath.txt` からパス読込み、ExMenu.xlsm の更新日時を比較 |
| ✓ | **SQL_Execution.bas** | `Open_oraconDB()` | Sub | ADO/ODBC で ricdb に接続 |
| ✓ | **SQL_Execution.bas** | `SQL_Exe()` | Sub | SQL 文を Execute 実行 |
| ✓ | **SQL_Execution.bas** | `SQL_INSERT_UPDATE()` | Sub | COUNT 判定後 INSERT または UPDATE |
|  | **SQL_Execution.bas** | `SQL_Delete()` | Sub | DELETE 実行（本ブックからは未呼出し） |
| ✓ | **SQL_Execution.bas** | `Disp_Sheet()` | Sub | SQL 結果を指定シートに書込み |
| ✓ | **SQL_Execution.bas** | `Set_Array()` | Sub | SQL 結果を配列に格納 |
| ✓ | **ファイル登録_取込.bas** | `AddSoft()` | Sub | 【ファイルの追加・消去】から呼出し。「AprList」シートを表示し C4 セルにフォーカス |
| ✓ | **ファイル登録_取込.bas** | `BackMenu()` | Sub | 「メニュー」シートへ戻る |
| ✓ | **ファイル登録_取込.bas** | `メニュー登録Main()` | Sub | 変更行の DB 登録・消去（資格確認後） |
| ✓ | **ファイル登録_取込.bas** | `メニュー取込()` | Sub | DB(ExSeihinj) から AprList I〜N 列へ読込み |
| ✓ | **ファイル登録_取込.bas** | `メニュー登録()` | Sub | 1 行分を ExSEIHINJ に INSERT/UPDATE |
| ✓ | **お知らせメッセージ.bas** | `メッセージ()` | Sub | お知らせファイル表示処理の入口 |
| ✓ | **お知らせメッセージ.bas** | `お知らせ()` | Function | サーバー/ローカルのお知らせ txt を比較・表示 |
| ✓ | **SikakuKakuninn.frm** | `CommandButton1_Click()` | Event | 資格確認フォームをキャンセルして閉じる |
| ✓ | **SikakuKakuninn.frm** | `Kakuninn_Click()` | Event | 社員番号/パスワードで DB 資格照合 |
|  | **SikakuKakuninn.frm** | `UserForm_Activate()` | Event | 入力欄クリア、`mpErrDes` に「資格確認」を設定 |
|  | **SikakuKakuninn.frm** | `UserForm_QueryClose()` | Event | フォームを Unload |
| ✓ | **SikakuKakuninn.frm** | `SyainName()` | Function | SHAINMST を SELECT し社員名を返す |
...
|---|---|---|---|---|---|
|   | Syainn | Label | 社員 | — | 社員番号ラベル |
|   | Label1 | Label | — | — | 補助ラベル |
| ✓ | SyainnNo | TextBox | — | — | 社員番号入力 |
| ✓ | PassWord | TextBox | — | — | パスワード入力 |
| ✓ | Kakuninn | CommandButton | 確認 | — | 資格照合実行 |
| ✓ | CommandButton1 | CommandButton | キャンセル | — | フォームを閉じる |

#### イベント一覧

| ✓ | イベント | 動作概要 |
|---|---|---|
| ✓ | `Kakuninn_Click()` | 🗄️ `SyainName()` で SHAINMST SELECT。成功時 `mpSyaName` 設定して Unload |
| ✓ | `CommandButton1_Click()` | フォームを Unload（登録中止） |
|   | `UserForm_Activate()` | SyainnNo/PassWord をクリア |
|   | `UserForm_QueryClose()` | Unload |
| ✓ | `SyainName()` | 🗄️ `SELECT TRIM(shaname) FROM SHAINMST WHERE shano=... AND shask=... AND hshika='1' AND (cshika='2' OR cshika='3')` |
...
| DSN 名 | UID | PWD | 用途 |
|---|---|---|---|
| `ricdb` | `ric` | `t6101` | ExSeihinj / ExSEIHINJ / SHAINMST へのアクセス |
...
|---|---|---|---|---|---|
|   | `ExSeihinj` | 参照 | メニュー取込時のアプリ一覧 SELECT | `KAISYACD`（9001〜9030） | `FOLDER`, `FILENAME`, `KAIBIKOU`, `HIKITORI`, `TOUDATE`, `TOUNAME` |
| ✓ | `ExSEIHINJ` | **参照＋更新** | メニュー登録/消去 | `KAISYACD` | 更新: `FOLDER`, `FILENAME`, `KAIBIKOU`, `HIKITORI`, `TOUDATE`, `TOUNAME`（`メニュー登録()` / **SikakuKakuninn.frm** 経由） |
|   | `SHAINMST` | 参照 | 資格確認 | `shano` | `shaname`, `shask`, `hshika`, `cshika` |
...
-- アプリ登録（メニュー登録 / SQL_INSERT_UPDATE 内部）
...
| 2 | 🖥️（ﾌｧｲﾙ登録 Click） | 📄 `メニュー登録Main()` | 確認 MsgBox |
...
olevba 解析結果（ExMenu.xlsm）:

| 種別 | キーワード | 内容 |
|---|---|---|
| AutoExec | `Workbook_Open` | ブックオープン時に自動実行。DB 接続・ファイルコピー・外部ファイル読込が発生 |
| AutoExec | `Workbook_BeforeClose` | ブック終了時にツールバーを元に戻す |
| AutoExec | `CommandButton1_Click` | UserForm ボタンクリックイベント |
| Suspicious | `Open` | `ExAprReadPath.txt`・`ExMenuからのお知らせ.txt` の読み書きに使用 |
| Suspicious | `Write` | `ExMenuからのお知らせ.txt` 更新時に使用（ユーザー操作に基づく） |
| Suspicious | `Output` | テキストファイル書込みモードで使用 |
| Suspicious | `FileCopy` | サーバーからローカルへ最大 30 アプリを自動コピー |
| Suspicious | `Call` | 各サブルーチン呼び出しで使用 |
| Suspicious | `Chr` | `Chr(13)` による改行文字生成（メッセージ整形） |
| Suspicious | Hex Strings | VBA バイナリ内のエンコード。実際のエンコード処理なし |
| Suspicious | Base64 Strings | VBA バイナリ内のエンコード。実際のエンコード処理なし |
| IOC | `163.59.144.156` | 旧サーバー IP（コメントアウト済み UNC パス内）。現在は `C:\ラジエ工業\ExRicSys\`〈EXメニュー配置フォルダ〉 |

| 注意 | 内容 |
|---|---|
| DB 認証情報 | 接続文字列に UID/PWD をハードコード: `DSN=ricdb;UID=ric;PWD=t6101` |
| ファイル自動コピー | 起動時にサーバーから最大 30 ファイルをローカルへ自動コピー |
| 社員パスワード | `SHAINMST.shask` を SQL 直結照合。ハッシュ化等の対策は不明 |
