...
| ✓ | No | シート名 | codeName | 用途 | 最大行 | 最大列 | 印刷範囲 |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| ✓ | No | 名前 | 参照先 | スコープ | 用途 |
| --- | --- | --- | --- | --- | --- |
| ✓ | 1 | Nyuuakbi | 入荷状況!`$H$3` | ブック | 入荷日入力セル |
| ✓ | 2 | Nyukabi | 入荷状況!`$P$5` | ブック | 入荷日（yyyymmdd形式） |
| ✓ | 3 | N_Hyouji | 入荷状況!`$C$8:$J$28` | ブック | 入荷表示エリア |
| ✓ | 4 | Zaiko | 入荷状況!`$T$8:$AC$307` | ブック | 在庫DBバッファ |
| ✓ | 5 | Ric2T | 入荷状況!`$N$9` | ブック | Ric2 線源設定日 |
| ✓ | 6 | Ric2V | 入荷状況!`$O$9` | ブック | Ric2 初期速度 |
| ✓ | 7 | Ric2HP | 入荷状況!`$P$9` | ブック | Ric2 初期HP/日 |
| ✓ | 8 | Ric3T | 入荷状況!`$N$10` | ブック | Ric3 線源設定日 |
| ✓ | 9 | Ric3M | 入荷状況!`$O$10` | ブック | Ric3 初期M_time |
| ✓ | 10 | Ric3PP | 入荷状況!`$P$10` | ブック | Ric3 初期PP |
| ✓ | 11 | Today2V | 入荷状況!`$Q$9` | ブック | Ric2 今日の速度（半減期補正） |
| ✓ | 12 | Today3M | 入荷状況!`$Q$10` | ブック | Ric3 今日のM_time（半減期補正） |
| ✓ | 13 | TodayHP | 入荷状況!`$R$9` | ブック | Ric2 今日のHP/日 |
| ✓ | 14 | TodayPP | 入荷状況!`$R$10` | ブック | Ric3 今日のPP/日 |
| ✓ | 15 | Syukkabi | 出荷実績!`$D$1` | ブック | 出荷日入力セル |
| ✓ | 16 | SyukkaSuu | 出荷実績!`$E$2` | ブック | 出荷合計数 |
| ✓ | 17 | Hyou1 | 出荷実績!`$B$4:$E$48` | ブック | 出荷表示テーブル（左） |
| ✓ | 18 | Hyou2 | 出荷実績!`$G$4:$J$49` | ブック | 出荷表示テーブル（右） |
| ✓ | 19 | SyukkaTB | 出荷実績!`$C$4:$E$48`,`$H$4:$J$49` | ブック | 出荷データクリア対象 |
| ✓ | 20 | DataN | 出荷実績!`$P$1` | ブック | DBレコード件数 |
| ✓ | 21 | DataTB | 出荷実績!`$N$4:$S$801` | ブック | DB作業テーブル |
| ✓ | 22 | Siki | 出荷実績!`$N$1` | ブック | VLOOKUP式（会社名逆引き） |
|  | 23 | Debug | 出荷実績!`$A$1` | ブック | デバッグ用 |
| ✓ | 24 | KaisiDay | 稼働時間!`$C$7` | ブック | 集計開始日 |
| ✓ | 25 | SyuuryouDay | 稼働時間!`$D$7` | ブック | 集計終了日 |
| ✓ | 26 | TotalTime | 稼働時間!`$E$7` | ブック | 総時間 |
| ✓ | 27 | Kekka | 稼働時間!`$D$9:$F$10` | ブック | 照射/停止時間結果 |
| ✓ | 28 | SyousyaTime | 稼働時間!`$D$9:$F$10` | ブック | 照射時間（Kekka と同一範囲） |
| ✓ | 29 | Nengappi | 稼働時間!`$C$7:$D$7` | ブック | 集計開始/終了日ペア |
|  | 30 | _xlnm._FilterDatabase | 稼働時間!`$B$48:$E$73` | ローカル | AutoFilter範囲 |
|  | 31 | _xlnm.Database | [1]Table登録!`$F$5` | ブック | 外部参照（ExDBファイル表示.xlsm） |
|  | 32 | TableName | [1]Table登録!`$C$5` | ブック | 外部参照テーブル名 |
|  | 33 | TableNameDB | [1]Table登録!`$D$5` | ブック | 外部参照DBテーブル名 |
|  | 34 | Souti | [1]Table登録!`$G$5` | ブック | 外部参照装置 |
|  | 35 | Setumei | [1]Table登録!`$H$5` | ブック | 外部参照説明 |
...
| ✓ | No | シート | ボタンテキスト | マクロ | VMLソース | 動作 |
| --- | --- | --- | --- | --- | --- | --- |
| ✓ | 1 | 「入荷状況」 | 終了 | `[0]!終了` | vmlDrawing1.vml | ブックを閉じる（最後の1ブックならExcel終了） |
| ✓ | 2 | 「出荷実績」 | 終了 | `[0]!終了` | vmlDrawing2.vml | ブックを閉じる（最後の1ブックならExcel終了） |
| ✓ | 3 | 「稼働時間」 | 集計 | `[0]!線源登録表示` | vmlDrawing3.vml | 指定期間の装置稼働時間をDB集計・計算 |
...
| ✓ | No | モジュール | プロシージャ名 | スコープ | 種別 | 行数 | DB | 呼出元 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ✓ | 1 | **ThisWorkbook** | `Workbook_BeforeClose()` | Private | Event | 4 |  | （自動） |
| ✓ | 2 | **ThisWorkbook** | `Workbook_Open()` | Private | Event | 9 |  | （自動） |
| ✓ | 3 | **Sheet1**（「出荷実績」） | `Worksheet_Change()` | Private | Event | 13 |  | （自動：「出荷実績」D1変更時） |
| ✓ | 4 | **Sheet2**（「入荷状況」） | `Worksheet_Change()` | Private | Event | 13 |  | （自動：「入荷状況」H3変更時） |
| ✓ | 5 | **BD_Read入出荷** | `出荷履歴データ()` | Public | Sub | 25 | ✓ | `出荷集計()` |
| ✓ | 6 | **BD_Read入出荷** | `Ric23HP_Zaiko()` | Public | Sub | 52 | ✓ | `入荷集計()` |
| ✓ | 7 | **ExFunction** | `ExchengeDay()` | Public | Function | 30 |  | `出荷履歴データ()` |
| ✓ | 8 | **ExFunction** | `ExchengeDATE()` | Public | Function | 18 |  | （汎用、本ファイル内未使用の可能性あり） |
| ✓ | 9 | **集計処理** | `出荷集計()` | Public | Sub | 100 | ✓ | `Sheet1.Worksheet_Change()` |
| ✓ | 10 | **集計処理** | `入荷集計()` | Public | Sub | 165 | ✓ | `Sheet2.Worksheet_Change()` |
| ✓ | 11 | **集計終了** | `終了()` | Public | Sub | 8 |  | シートボタン「終了」 |
| ✓ | 12 | **表クリア** | `画面クリア()` | Public | Sub | 16 |  | `Workbook_Open()` |
| ✓ | 13 | **SQL_Execution** | `Open_oraconDB()` | Public | Sub | 10 | ✓ | 各DB操作プロシージャ |
| ✓ | 14 | **SQL_Execution** | `SQL_Exe()` | Public | Sub | 14 | ✓ | `Disp_Sheet()`, `Set_Array()`, `SQL_INSERT_UPDATE()` |
| ✓ | 15 | **SQL_Execution** | `SQL_INSERT_UPDATE()` | Public | Sub | 38 | ✓ | （汎用：本ファイル内で直接呼出なし） |
| ✓ | 16 | **SQL_Execution** | `SQL_Delete()` | Public | Sub | 16 | ✓ | （汎用：本ファイル内で直接呼出なし） |
| ✓ | 17 | **SQL_Execution** | `Disp_Sheet()` | Public | Sub | 48 | ✓ | `出荷履歴データ()`, `Ric23HP_Zaiko()` |
| ✓ | 18 | **SQL_Execution** | `Set_Array()` | Public | Sub | 28 | ✓ | `稼働状況()`, `線源登録表示()` |
| ✓ | 19 | **装置稼働状況** | `稼働状況()` | Public | Sub | 58 | ✓ | （外部呼出用：EXメニュー等） |
| ✓ | 20 | **装置稼働状況** | `ExHenkan()` | Public | Function | 9 |  | `稼働状況()` |
| ✓ | 21 | **運転時間** | `線源登録表示()` | Public | Sub | 166 | ✓ | シートボタン「集計」 |
| ✓ | 22 | **運転時間** | `ExHenkan()` | Public | Function | 9 |  | `線源登録表示()` |
| ✓ | 23 | **運転時間** | `ChengTime()` | Public | Function | 3 |  | `線源登録表示()` |
...
| No | DSN | UID | PWD | 使用モジュール | 備考 |
| --- | --- | --- | --- | --- | --- |
| 1 | ricdb | ric | t6101 | **BD_Read入出荷**（ExKanriTB, tokumst, zaiko） | — |
...
| ✓ | No | テーブル名 | 操作 | 使用プロシージャ | 用途 |
| --- | --- | --- | --- | --- | --- |
|  | 1 | syukar | SELECT | `出荷履歴データ()` | 出荷履歴（日次出荷データ） |
|  | 2 | tokumst | SELECT | `出荷履歴データ()` | 得意先マスタ（会社コード⇔会社名） |
|  | 3 | ExKanriTB | SELECT | `Ric23HP_Zaiko()` | 装置管理テーブル（線源設定値） |
|  | 4 | zaiko | SELECT | `Ric23HP_Zaiko()` | γ在庫テーブル（Oracle / DSN=ricdb / UID=ric）。※名前付き範囲 `Zaiko`（入荷状況!$T$8:$AC$307）は本テーブルの SELECT 結果を一時格納するシート上のバッファ |
|  | 5 | zaikor | SELECT | `Ric23HP_Zaiko()` | γ在庫テーブル（Oracle / DSN=ricdb / UID=rich） |
|  | 6 | sengnr1 | SELECT | `稼働状況()`, `線源登録表示()` | 1号機線源制御ログ |
|  | 7 | kyouj2 / KYOUJ2 | SELECT | `稼働状況()`, `線源登録表示()` | 2号機共用記録 |
|  | 8 | sengnr3 | SELECT | `稼働状況()`, `線源登録表示()` | 3号機線源制御ログ |
...
| No | モジュール | 接続文字列 |
| --- | --- | --- |
| 1 | **BD_Read入出荷** | `DSN=ricdb;UID=rich;PWD=t6101` |
| 2 | **BD_Read入出荷** | `DSN=ricdb;UID=ric;PWD=t6101` |
| 3 | **運転時間** | `DSN=ricdbh;UID=ric;PWD=t6101` |
...
| No | プロシージャ | リスク箇所 |
| --- | --- | --- |
| 1 | `出荷履歴データ()` | `WHERE syudate='{mySyuDay}'` — `Syukkabi` セル値を `ExchengeDay()` で変換後に連結 |
| 2 | `Ric23HP_Zaiko()` | `WHERE nyukabi={myNyukaBi}` — `Nyukabi` セル値をクォートして連結 |
| 3 | `線源登録表示()` | `WHERE sdate |
