...
|   | `$K$4` | — | 設定値 | "会社ｺｰﾄﾞ" | 補助テーブルヘッダー（非表示列） |
...
|   | `$N$5:$N$13` | — | 設定値 | 引取 / 混載便 / 保管品 / ﾁｬｰﾀ便 / 納品 / 品証扱い / γ扱い / 営業扱い / その他 | 出荷方法の選択肢マスタ（9種） |
...
| K | 会社ｺｰﾄﾞ | `sehmst.kaisyacd` | 非表示・HouTB |
...
> **注意**: 現在のファイルではボタンオブジェクトが消失しています（drawing XML にボタン定義なし）。
> 以下は検証版ファイル（★検証_Ex出荷方法報告書発行登録.xlsm）で確認されたボタン構成と、VBA コード内の対応プロシージャから推定したマッピングです。
...
| `業者名と引取抽出()` | **デーた抽出.bas** | ① ボタン【出荷方法/報告書発行種別表示】※消失 ② `Workbook_Open()` から Call | ボタン + 内部Call |
| `引取報告書登録()` | **登録.bas** | ボタン【変更を登録する】※消失 | ボタン |
| `Bookを閉じる()` | **終了処理.bas** | ボタン【終了】※消失 | ボタン |
...
| # | 起点 | 処理 | 結果 |
|---|---|---|---|
| 1 | 📊（変更を登録する ボタン Click）※ボタン消失 | 📄 `引取報告書登録()` / **登録.bas** | 差分検出ループ開始 |
...
| # | 起点 | 処理 | 結果 |
|---|---|---|---|
| 1 | 📊（終了 ボタン Click）※ボタン消失 | 📄 `Bookを閉じる()` / **終了処理.bas** | アラート非表示 |
