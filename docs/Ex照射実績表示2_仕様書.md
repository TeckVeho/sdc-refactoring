...
| No | シート名 | 状態 | VBA制御 |
| --- | --- | --- | --- |
| 1 | 「実績表示」 | 表示（Visible） | `Protect UserInterfaceOnly:=True` でシート保護（VBA操作は許可） |
| 2 | 「実績」 | 非表示（Hidden） | VBAがDBデータを書き込むワーク領域 |
| 3 | 「社員」 | 非表示（Hidden） | VBAが社員マスタを書き込むワーク領域 |
...
| ✓ | No | モジュール | プロシージャ | スコープ | 種別 | 行数 | 用途 |
| --- | --- | --- | --- | --- | --- | --- | --- |
|  | 1 | **ThisWorkbook** | `Workbook_BeforeClose()` | Public | Private Sub | 3 | 保存確認なしでブックを閉じる |
| ✓ | 2 | **ThisWorkbook** | `Workbook_Open()` | Public | Private Sub | 7 | 📊シート保護設定 → `GamenCls1()` → `社員データ()` 呼び出し |
| ✓ | 3 | **Sheet2**（「実績表示」） | `Worksheet_Change()` | Public | Private Sub | 12 | 📊`Uno`セル変更時、空なら画面クリア / 値ありなら`起動Main()` |
|  | 4 | **Utility** | `aaaaa()` | Public | Sub | 3 | イベント有効化（デバッグ用） |
| ✓ | 5 | **画面クリア** | `GamenCls1()` | Public | Sub | 37 | 🖥️全名前付き範囲を空文字にクリア、PrintAreaリセット |
| ✓ | 6 | **画面クリア** | `GamennCls2()` | Public | Sub | 30 | 🖥️実績表示領域とヘッダ（Uno以外）をクリア |
| ✓ | 7 | **起動** | `起動Main()` | Public | Sub | 21 | 📄メイン制御：`社員データ()` → `在庫実績データ()` → `在庫表示()` → `実績表示()` |
| ✓ | 8 | **表示** | `在庫表示()` | Public | Sub | 48 | 🖥️「実績」シートから在庫データを読み取り「実績表示」シートに埋め込み |
| ✓ | 9 | **表示** | `実績表示()` | Public | Sub | 96 | 🖥️「実績」シートから実績データを読み取り「実績表示」シートに一覧表示 |
|  | 10 | **終了処理** | `Bookを閉じる()` | Public | Sub | 8 | 🖥️メッセージ非表示で閉じる（最後の1ブックならExcel終了） |
| ✓ | 11 | **SQL_Execution** | `Open_oraconDB()` | Public | Sub | 9 | 🗄️ADODB.Connectionを使ったODBC接続 |
| ✓ | 12 | **SQL_Execution** | `SQL_Exe()` | Public | Sub | 11 | 🗄️SQL文実行（Execute） |
| ✓ | 13 | **SQL_Execution** | `SQL_INSERT_UPDATE()` | Public | Sub | 34 | 🗄️INSERT/UPDATE汎用処理（キー存在チェック付き） |
| ✓ | 14 | **SQL_Execution** | `SQL_Delete()` | Public | Sub | 14 | 🗄️DELETE汎用処理 |
| ✓ | 15 | **SQL_Execution** | `Disp_Sheet()` | Public | Sub | 43 | 🗄️SQL実行結果をシートに貼り付け |
| ✓ | 16 | **SQL_Execution** | `Set_Array()` | Public | Sub | 27 | 🗄️SQL実行結果を配列に格納 |
| ✓ | 17 | **在庫実績Read** | `在庫実績データ()` | Public | Sub | 107 | 🗄️在庫・実績データのSQL構築とDB取得 |
| ✓ | 18 | **社員データRead** | `社員データ()` | Public | Sub | 16 | 🗄️社員マスタのDB取得 |
...
| No | カテゴリ | 内容 | リスク |
| --- | --- | --- | --- |
| 1 | DB認証情報のハードコード | `DSN=ricdb;UID=ric;PWD=t6101` がVBAソースに直書き | 中：環境変数・設定ファイル等への外部化を推奨 |
| 2 | SQLインジェクション | `Uno` の値をそのまま文字列結合でSQL構築（`WHERE uno='..." & myUno & "'`） | 中：パラメータ化クエリの使用を推奨 |
| 3 | エラー処理での `Stop` | `SQL_Exe()` 内でエラー時に `Stop` が実行される（開発環境ではブレーク、実行環境では実行時エラー） | 中：本番環境では `Stop` を除去しログ出力に変更を推奨 |
| 4 | シート保護 | `UserInterfaceOnly:=True` でUI操作は制限されるが、VBAパスワード保護なし | 中：VBAプロジェクトのパスワード保護を推奨 |
