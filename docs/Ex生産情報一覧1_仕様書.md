...
|---|---|---|---|---|
| 未処理品一覧 | — | 常時表示 | — | — |
| 受付番号別完了 | あり | `受付番号別完了遷移()` 実行時（「未処理品一覧」上の矢印図形クリック。2号機完了予想モード時のみ矢印が表示される） | `未処理品一覧に戻る()` 実行時 | **画面クリア_遷移1.bas** / **シェープ表示非表示.bas** |
...
| シート | セル | 種別 | 制約 |
|---|---|---|---|
| 「未処理品一覧」 | `R16:R1015` | リスト | リスト: `SyukkaHou` |
| 「受付番号別完了」 | `AE2` | 整数 | 1〜300 |
| 「Ric3TR」 | `AW3` | リスト | リスト: `$AW$6:$AW$7` |
| 「設定」 | `AE4:AE204` | 整数 | 1〜999 |
...
|   | 未処理品一覧 | フィルタ ON | `オートフィルター()` | オートフィルター ON/OFF |
...
| ✓ | 未処理品一覧 | 詳細/表示（矢印図形） | `受付番号別完了遷移()` | 「受付番号別完了」シートへ遷移（2号機完了予想モード時のみ表示） |
| ✓ | 受付番号別完了 | 戻 | `未処理品一覧に戻る()` | 「未処理品一覧」シートへ戻る |
|   | 受付番号別完了 | （印刷範囲設定図形） | `印刷範囲図形より設定()` | 印刷範囲を図形位置から設定 |
