...
/** GET /api/dosimetry/jmm90?uno= — JMM90φ 記入用紙データ（受付マスタ読込） */
jmm90Router.get("/", requireAuth, async (req, res) => {
  ...
  try {
    const data = await loadJmmPayload(prisma, parsed.data.uno, "90");
    if (!data) {
