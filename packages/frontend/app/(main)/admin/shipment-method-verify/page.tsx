...
export default function ShipmentMethodVerifyPage() {
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      const res = await apiFetch<Payload>("/api/admin/shipment-method-verify");
      setData(res.data);
    } catch (e) {
      setData(null);
  ...
  return (
    <div className="space-y-4 p-4">
      <div className="rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950">
        <strong>検証ビュー</strong>（読取専用）— 本番の出荷方法データと同一ソースです。操作は「出荷方法報告書」画面で行ってください。
      </div>
      <div>
        <h1 className="text-lg font-semibold tracking-tight">出荷方法（検証）</h1>
